...
```bash
# Postgres (bitnami/postgresql-ha chart as of Phase 17 — see that section):
# db "catalog", user "catalog", 3-node repmgr cluster behind 2 pgpool routers.
# Connect via pgpool (postgres-pgpool.data-platform.svc.cluster.local:5432),
# never the raw postgres-postgresql service directly, for automatic-failover
# routing. The catalog user's password lives in the `postgres-credentials`
# Secret's `password` key (unchanged since Phase 12); the HA cluster's own
# superuser/repmgr/pgpool-admin passwords live in `postgres-ha-credentials`
# (Phase 17). Retrieve with:
#   kubectl -n data-platform get secret postgres-credentials -o jsonpath='{.data.password}' | base64 -d; echo              # catalog user
#   kubectl -n data-platform get secret postgres-ha-credentials -o jsonpath='{.data.postgres-password}' | base64 -d; echo  # HA cluster superuser
...
## Multi-tenancy in the data model (Phase 14 — BUILD_PLAN_COMMERCIAL.md)

`Dataset`, `File`, and `Feature` all got a `tenant_id` column, and every `/public/*` query now filters on it — Phase 13 gave requests an identity, this phase is what actually stops tenant A from reading tenant B's data. `Dataset.tenant_id` is set explicitly (the authenticated caller's tenant on `/public/upload*`/`/pull/*`, or the internal API's `create_dataset` payload); `File.tenant_id`/`Feature.tenant_id` are always inherited from the parent `Dataset` at creation time (`catalog/api.py`'s `create_file`/`create_feature` look the dataset up and copy it) rather than requiring every internal caller to know or pass a tenant — `connectors/base.py`'s `Connector.run()` and every existing CronWorkflow needed **zero changes** as a result.

**Two tenants, not an unbounded set, by design**: a request can see its own `tenant_id` plus the reserved `platform` tenant (`_visible_tenants()` in `catalog/public_api.py`) — `platform` holds connector-synced public reference data (ChEMBL/PubChem/UniProt syncs, local dev fixtures) that every tenant should be able to browse, since it isn't any specific customer's private data. What's actually isolated is *another specific tenant's submitted data* (their uploads/pulls) — that's the case that must never leak, and does not. `/stats/pipeline-phases`, `/stats/queue-depth`, and `/stats/cache-hit-rate` are deliberately **not** tenant-scoped (aggregate platform-operational metrics, not individual records — scoping them would make the demo's own dashboard only show a handful of its own submissions instead of real platform activity); `/stats/rejected-files` **is** scoped, since it returns individual File rows.

**No Alembic yet (that's Phase 17), so this was a hand-written migration against the live database**, not a code-only change: `ALTER TABLE ... ADD COLUMN tenant_id` (nullable first) on all three tables, a backfill (`source IN (chembl, pubchem, uniprot, local) → 'platform'`, everything else → `'public-demo'`, since every existing row predated real tenants), then `SET NOT NULL` + indexes — all in one transaction, run directly via `kubectl exec postgres-postgresql-0 -- psql ...` before deploying the new catalog image (deploying first would have crashed on any query assuming the column already existed and was populated).

**Verified against the live cluster with a genuinely new tenant, not a mock**: issued a real `acme-corp` API key, uploaded a real file under it, and confirmed the `public-demo` tenant's key gets a 404 (not empty results, not a 403 that would confirm existence) on that dataset, that specific file, its raw-content preview, its upload-status poll, and the feature it produced — while `acme-corp`'s own key sees all of them, correctly tagged `tenant_id: "acme-corp"`, and can also browse the shared `platform` sources (chembl/pubchem/uniprot/local) despite having zero data of its own in them. Test tenant and data cleaned up afterward.

## Network & pod security hardening (Phase 16 — BUILD_PLAN_COMMERCIAL.md)

**NetworkPolicy is written but NOT enforced on this cluster — proven empirically, not assumed.** This cluster's CNI is kindnet (kind's default), and kindnet does not implement NetworkPolicy. Verified directly: two throwaway pods, a default-deny-all ingress policy applied between them, and traffic still flowed through unblocked. `infra/k8s-manifests/network-policies.yaml` is correct and ready for a real environment (a managed cluster, or kindnet + Calico) — default-deny ingress at the namespace level, then explicit allows so only the pods that actually need to reach Postgres/MinIO/NATS/catalog can (labeled via a new `network-role` pod label — `engine-step`/`connector-sync`/`backup` — added via `podMetadata.labels` on the relevant WorkflowTemplate/CronWorkflows specifically so policies have something stable to select on). Scope is deliberately ingress-only, not a namespace-wide default-deny-egress: Prometheus/Grafana/Loki's internal scrape/query graph is real and non-trivial, and writing an untested egress allow-list for it — with no way to verify it against real enforcement — risked shipping something that looks complete but would silently break observability the moment enforcement is ever turned on. **Re-run the same empirical test (default-deny + a raw connection attempt between two pods) before trusting these provide real isolation, the moment this cluster's CNI changes** — don't assume `kubectl get networkpolicy` showing them applied means anything is actually blocked.

**securityContext (non-root, no privilege escalation, dropped capabilities, read-only root filesystem where safe) + resource limits + a real liveness probe, verified live against every workload, not just applied and assumed**:
- `catalog` (`infra/k8s-manifests/catalog.yaml`): full hardening including `readOnlyRootFilesystem: true` (Starlette's `UploadFile` can spool large uploads to disk, hence the `/tmp` emptyDir mount). The liveness probe was proven two different ways: killing the container's real PID 1 from the node (bypassing `kubectl exec`'s own PID-namespace isolation — a plain `kubectl exec ... kill 1` only kills the *exec session's* PID 1, not the container's, a real gotcha hit while testing this) triggered a plain restart-on-exit; `SIGSTOP`-freezing the real process (still alive, just unresponsive) produced the actual target event — `Killing: Container catalog failed liveness probe, will be restarted` — which is the specific "hung, not crashed" failure mode a `readinessProbe` alone could never recover from.
- `engine-steps`/`connectors`/`backup` Argo pods (`ingest-validate-template.yaml`'s three steps, both connector CronWorkflows, `nightly-backup-cronworkflow.yaml`): same hardening, all previously had **no** `resources` or `securityContext` at all. Verified with real molecule+sequence files through the full DAG, a real ChEMBL/PubChem sync, and a real nightly backup run — all succeeded under the hardened containers, confirmed by inspecting the actual running pod spec (Argo's `wait` sidecar container is untouched by this — it's the emissary executor's own container, not something a WorkflowTemplate's `securityContext`/`resources` fields reach).
- Third-party charts: Postgres, Argo's controller, and (already, by its own chart defaults) KubeRay's operator were already fully hardened — only missing `resources.limits`, added. MinIO got dropped capabilities + limits (its `readOnlyRootFilesystem: false` default was left alone — the chart's own choice, not verified safe to override). Prometheus/Grafana/Loki/node-exporter/kube-state-metrics were already hardened by their charts' own defaults; only kube-state-metrics/node-exporter/prometheus-operator were missing `resources.limits`.
- **NATS needed a real fix, not just new values — this is the one that actually broke, live, and is worth understanding before touching this file again.** NATS shipped with zero securityContext at any level. Flipping straight to non-root failed hard: `[FTL] Can't start JetStream: storage directory is not writable`, confirmed via `CrashLoopBackOff` on the real pod — the existing JetStream PVC had real data on it, written while the pod ran as root, and `fsGroup` alone does not reliably fix *permission bits* on a directory that was created `700` with no group access (it only changes group *ownership*, not necessarily the mode bits, depending on how the directory was originally created). Rolled back immediately (verified `PLATFORM_EVENTS`'s existing 116 messages were untouched), then fixed properly with a one-time `initContainer` (`chown -R 1000:1000 /data`, running as root since init containers can have their own security context independent of the main container) before the non-root main container starts — the standard, correct pattern for this exact situation. Re-verified with a real ChEMBL sync afterward: message count went 116 → 119, proving the volume is genuinely writable now, not just that old data is still readable.
- Ray head/worker got non-root + dropped capabilities, but **not** `readOnlyRootFilesystem` — Ray writes session/object-store scratch data to `/tmp/ray` by default, and `raycluster.yaml`'s own header comment already documents why a custom `command`/`args` (which redirecting `--temp-dir` would require) can't be set without breaking KubeRay's own `ray start` invocation (a Phase 8 gotcha). Verified via `ray status` showing healthy 2-worker resource reporting after a forced pod recreation (`kubectl delete pods -l ray.io/cluster=raycluster` — the same manual-recreation requirement Phase 8 already documented for image reloads; a `RayCluster` CRD spec change alone did not trigger it).

**PodDisruptionBudgets were not added** — the plan's own trigger for these is "where more than one replica exists," and nothing in this cluster runs with `replicas > 1` yet (`catalog` deliberately stays at 1 until Phase 21's shared rate-limit store makes scaling it safe). Nothing to protect yet; add these when Phase 17 or 21 actually introduces multi-replica services.

## High availability & disaster recovery (Phase 17 — BUILD_PLAN_COMMERCIAL.md)

**Postgres: standalone → `bitnami/postgresql-ha`, 3 repmgr nodes + 2 pgpool routers, real automatic failover — proven by actually killing the primary, not just deploying the chart.** `infra/helm/postgres-ha-values.yaml` replaces the retired `infra/helm/postgres-values.yaml` (kept in the repo with a header noting it's retired, not deleted, as a record of what Phase 12 originally deployed). `fullnameOverride: postgres` reproduces the old `postgres-postgresql` hostname for direct node access and adds `postgres-pgpool` as the new **app-facing, failover-aware** endpoint — every consumer (`postgres-credentials`'s `database-url` key, `nightly-backup-cronworkflow.yaml`'s `POSTGRES_HOST`, `grafana-postgres-datasource.template.yaml`'s `url`) now points at pgpool, never the raw postgresql service (which round-robins across all 3 nodes including replicas that don't accept writes). Cutover was dump → uninstall → reinstall → restore → verify, not an in-place upgrade — a hand-written `pg_dump` taken and row-counted (31 datasets/53 files/18 features/4 api_keys) before touching anything, restored afterward with an exact row-count match down to every table, migrated forward via Alembic (`alembic stamp head`, see below) so nothing needed hand-written SQL.

**Real, non-obvious failures hit standing this up — all fixed, all worth knowing before touching this chart again:**
1. `bitnami/postgresql-repmgr` and `bitnami/pgpool` 404 on the free `bitnami/` Docker Hub org as of the August 2025 Bitnami Secure Images cutover — pinned version tags were pulled from the free tier. `bitnamilegacy/` is Bitnami's own replacement org still carrying the same tags for free (confirmed via real `docker pull` against both orgs first); `postgres-ha-values.yaml`'s `postgresql.image.repository`/`pgpool.image.repository` override to it.
2. `bitnami/postgresql-ha` only ships Postgres **17.6**, not 18 (the retired standalone chart's version) — no chart version currently packages an 18 build. A real downgrade, accepted deliberately: nothing in this schema/query layer uses PG18-specific behavior, so it's functionally harmless here, but it's a real fact about this cluster now, not a detail to forget.
3. Without explicitly setting `postgresql.username: catalog` / `postgresql.database: catalog`, the chart's initdb-time bootstrap creates only a default `postgres` superuser/database — not the `catalog` role the app actually connects as. Since this only runs once against an empty data directory, catching it after the fact meant wiping the (still-empty, no-real-data-yet) cluster and reinstalling correctly rather than patching around it.
4. **pgpool's read/write load-balancing broke `catalog/api.py`'s own read-your-own-write pattern** (`db.commit()` immediately followed by `db.refresh()`) — the post-commit refresh SELECT landed on a standby that hadn't yet replicated the just-committed INSERT, a genuine race (`disableLoadBalancingOnWrite: transaction` only pins reads *inside* the same transaction as a write; a follow-up statement after commit is a new transaction, free to land anywhere). This app has no read-scaling need pgpool's load balancing would actually serve, so `pgpool.useLoadBalancing: false` is the fix — all traffic goes to the primary; automatic failover is unaffected since it's a separate mechanism (repmgr, not pgpool's load balancer).
5. Whenever pgpool's pods are replaced (e.g. a values change), **restart the catalog Deployment too** — its SQLAlchemy connection pool holds sockets to the old pgpool pods, and a stale pooled connection surviving the replacement produced the exact same "write succeeded, refresh failed" symptom as (4) even after the real fix was deployed, until the pool was recycled.

**The failover drill (the actual done-when), done for real**: with the cluster carrying live data, a real dataset write through the actual catalog API, confirmed replicated to both standbys, then `kubectl delete pod postgres-postgresql-0 --grace-period=0 --force` against the real primary. repmgr auto-promoted `postgres-postgresql-1` in **~14 seconds** with zero manual steps; pgpool detected the new primary and routed to it transparently — a second real write through the catalog API succeeded immediately after, with no restart or reconfiguration. Both the before- and after-kill writes were present afterward; row counts matched exactly. The old primary rejoined automatically as a standby once it came back (repmgr auto-detected the new primary and re-cloned).

**MinIO: standalone → 4-node distributed (erasure-coded), same release name, zero consumer changes.** `infra/helm/minio-values.yaml` adds `mode: distributed`, `replicas: 4`, `drivesPerNode: 1` — unlike Postgres, the chart, release name, and therefore the `minio.data-platform.svc.cluster.local` hostname are all unchanged, so no consumer (connectors, engine steps, backup script, Argo templates) needed touching. Cutover: `mc mirror` all 8 buckets (7 declared zones + `backups/`, previously only ever created ad hoc by `backup.sh`'s own `mc mb --ignore-existing` — now declared explicitly in the chart's `buckets:` list) to local disk as a snapshot, uninstall, reinstall under the same release name, mirror back in. Verified with exact object-count matches per bucket and a zero-diff `mc mirror --dry-run` in both directions, then a real file pushed through the actual ingest→validate→transform Argo DAG against the new backend (this also drained a 20-workflow backlog of never-consumed historical connector-sync events sitting on `PLATFORM_EVENTS` — expected given `engine/ingestion_service.py` is still a manual batch per Phase 2/7's notes, not something this migration caused).

**Alembic finally replaces `Base.metadata.create_all()`, unblocking every future schema change** — `catalog/alembic/` (env.py reads `DATABASE_URL` from the environment, same variable `catalog/db.py` requires), one baseline migration matching the schema `create_all()` used to produce, applied to the live DB via `alembic stamp head` (not `upgrade` — the tables already existed). `catalog/api.py`'s `lifespan()` hook is gone entirely; migrations now run via a new `migrate` initContainer on the catalog Deployment (`alembic upgrade head`, same initContainer pattern Phase 16 already established for NATS's chown fix), so the main container never touches schema. Proved the mechanism with a real, useful migration, not a synthetic one: `Feature.created_at` is filtered/ordered on by `GET /features?produced_since=&produced_before=` but had no index — added via a real Alembic migration against the live DB with existing rows, verified via `pg_indexes` and an unchanged row count.

**Backups: mechanism re-verified against both new backends, RTO/RPO stated explicitly for the first time, off-box gap still open.** `nightly-backup-cronworkflow.yaml`'s `POSTGRES_HOST` now points at pgpool; no MinIO-side change was needed. A real manually-triggered backup run succeeded against the live HA/distributed stack. **RTO ≈ 1 hour, RPO ≈ 24 hours** (bounded by the nightly schedule) — first-pass numbers given today's small data volume, not a permanent commitment; revisit as real data volume grows. Verified with a genuine timed restore drill, same bar Phase 10 already set: the actual nightly backup artifacts downloaded and restored into fresh, disconnected `docker run` Postgres and MinIO containers (no connection to the kind cluster) — Postgres restore took ~26 seconds with an exact row-count match against the live catalog across every table; the restored MinIO object was byte-for-byte identical to the source and parsed back into valid tensor JSON. Both containers torn down afterward. **Off-box backup storage is still an open gap** — no external cloud account is available in this environment (same situation Phase 10 already documented); the trigger for closing it is the same as before: a second object-storage account/region/provider becoming available.

**Not done, noted rather than silently skipped**: PodDisruptionBudgets for the newly multi-replica `postgres-ha`/pgpool/`minio` StatefulSets/Deployments — the trigger this file's own Phase 16 section named ("more than one replica exists") has now fired for these three, but adding PDBs wasn't in this phase's stated done-when and wasn't done speculatively; a reasonable next small task, not assumed complete here.
...
**v2's phases (0–11) are done.** `docs/BUILD_PLAN_COMMERCIAL.md`'s Phase 12 (secrets), Phase 13 (per-customer auth), Phase 14 (tenant isolation), Phase 16 (network/pod security hardening), and Phase 17 (HA/DR) are also done; Phases 18–22 are not yet started. Follow that document's own suggested order (18 → 19 → 15 → 20 → 21 → 22) the same way v2's phases were followed in sequence — each of its phases also has an explicit "done when" acceptance check.
