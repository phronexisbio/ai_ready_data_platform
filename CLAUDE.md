...
**Cutover broke the live public site on purpose, not by accident — and needed a manual follow-up this session couldn't finish**: the moment the old `PUBLIC_API_KEY` check was replaced, the Vercel-hosted frontend's `CATALOG_API_KEY` env var (still holding the old static value) started getting 401s. A new `public-demo`-tenant key was issued and `.env.local` updated for local dev, but **the production Vercel environment variable needs updating by hand** — no Vercel CLI/token is available from this environment. Check `engine.phronexis.bio` is actually working after that update, not just assume it.
