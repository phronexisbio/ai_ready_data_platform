...
**v2's phases (0–11) are done.** `docs/BUILD_PLAN_COMMERCIAL.md`'s Phase 12 (secrets), Phase 13 (per-customer auth), and Phase 14 (tenant isolation) are also done; Phases 16–22 are not yet started. Follow that document's own suggested order (16 → 17 → 18 → 19 → 15 → 20 → 21 → 22) the same way v2's phases were followed in sequence — each of its phases also has an explicit "done when" acceptance check.
